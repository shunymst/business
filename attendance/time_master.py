...
def get_minute_work_rest_time(str_start_time, str_end_time, rest_time_list):
    ...
    for i, rest_time in enumerate(rest_time_list):
        print(rest_time["start_time"], rest_time["end_time"])
        rest_start_time = datetime.datetime.strptime(rest_time["start_time"], "%H:%M:%S")
        rest_end_time = datetime.datetime.strptime(rest_time["end_time"], "%H:%M:%S")
